...
class Event(BaseModel):
    ...
    probability: Optional[float] = Field(..., json_schema_extra={"min": 0, "max": 1})
    description: str
    starts: Optional[int]
    resolve_date: Optional[int]
    cutoff: Optional[int]
    retries: int = Field(2, json_schema_extra={"min": 0, "max": 1})
    next_try: Optional[int]
    prev_prob: Optional[float] = Field(..., json_schema_extra={"min": 0, "max": 1})
