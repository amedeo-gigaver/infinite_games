...
class EventAggregator:
    ...
    async def create(
        cls,
        state_path: str,
        integrations: List[Type[ProviderIntegration]],
        db_path="database.db",
    ):
        ...
        while tried < tries:
            try:
                ...
                if "locked" in str(e):
                    ...
                    bt.logging.error(traceback.format_exc())
                    bt.logging.error(f"Error sync miner predictions {blocktime} ")
                    break
            except Exception as e:
                bt.logging.error(e)
                bt.logging.error(traceback.format_exc())
        ...
        while tried < tries:
            try:
                ...
                if "locked" in str(e):
                    ...
                    bt.logging.error("Error marking submissions!")
                    bt.logging.error(e)
                    bt.logging.error(traceback.format_exc())
                    break
            tried += 1
